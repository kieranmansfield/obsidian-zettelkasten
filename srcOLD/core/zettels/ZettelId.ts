...
import { deepSplitSegment, buildZettelId, generateTimeStampId } from './id-helpers'
...
export default class ZettelId {
  ...
  nextChild(): ZettelId {
    ...
    // if (/^[0-9]+$/.test(last)) {
    //   // last segment is a number, append letter segment 'a'
    //   newSegments.push('a')
    //   return new ZettelId(this.timestamp, newSegments)
    // } else {
    //   // last segment is letters, append number '1'
    //   newSegments.push('1')
    //   return new ZettelId(this.timestamp, newSegments)
    // }
  }
  ...
  nextSibling(): ZettelId | null {
    ...
    // const parent = new ZettelId(this.timestamp, this.segments.slice(0, -1))
