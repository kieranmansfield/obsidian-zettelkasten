...
export class SequenceNavigatorView extends ItemView {
		...
		allFiles.forEach((file) => {
			...
			if (childIdWithoutPrefix.startsWith(parentIdWithoutPrefix)) {
				...
				if (this.isDirectChild(suffix)) {
					const childLevel = this.calculateLevel(
						childIdWithoutPrefix,
					);
					const parentLevel = this.calculateLevel(
						parentIdWithoutPrefix,
					);
	...
	private renderNode(
		...
		if (hasChildren) {
			...
			const iconName = node.isCollapsed
				? "chevron-right"
				: "chevron-down";
