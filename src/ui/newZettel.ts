import { EditorPosition, MarkdownView, Notice, Plugin, TFile } from 'obsidian'
import { NewZettelModal } from './NewZettelModal'
import { LuhmanSettingTab } from './LuhmanSettingTab'
import { ZettelSuggester } from './ZettelSuggester'

import { NewZettelSettings } from '../types/interfaces'
...
const checkSettingsMessage = 'Please check the Zettel ID format and separator in settings.'
...
export default class NewZettel extends Plugin {
  ...
  async makeNote(
    ...
    if (useTemplate) {
      ...
      const testTitle =
        this.settings.templateRequireTitle == false || titleRegex.test(template_content)
      const testLink =
        this.settings.templateRequireLink == false || backlinkRegex.test(template_content)
      ...
      callback: async () => {
        ...
        new ZettelSuggester(this.app, titles, this.currentlySelectedText(), (file) => {
          ...
          if (doInsert == undefined)
            new Notice('Error inserting link, Code: 6a46de1d-a8da-4dae-af41-9d444eaf3d4d')
          else doInsert()
        }).open()
      },
    })
  ...
  async loadSettings() {
    this.settings = Object.assign({}, DEFAULT_NEWZETTEL_SETTINGS, await this.loadData())
  }
