...
import { SequenceReorderModal } from './modal/SequenceReorderModal'

import { SequenceNode, ParentNote } from '../types/interfaces'
...
export class NoteSequencesView extends ItemView {
  ...
  private async handleSequenceReorder(
    ...
    for (let i = 0; i < promoted.length; i++) {
      ...
      await this.plugin.app.fileManager.renameFile(promotedFile, tempPath)
    ...
    for (const promotedFile of promoted) {
      ...
      if (grandparentId) {
        const newId = await this.generateChildZettelId(grandparentId, folder, promotedFile)
        const newPath = `${folder.path}/${newId}.md`
        await this.plugin.app.fileManager.renameFile(promotedFile, newPath)
      } else {
    ...
    for (const note of reorderedNotes) {
      ...
      if (level === 1) {
        ...
        if (currentLetter === 'z' && counterMap.has(noteParentId)) {
          new Notice('Error: Maximum of 26 children (a-z) reached for this parent')
          continue
        }
        ...
        if (!lastLevel1Id) {
          new Notice('Error: Cannot create level 2 child without a level 1 parent')
          continue
        }
        ...
        counterMap.set(noteParentId, (parseInt(currentNumber) + 1).toString())
        lastLevel2Id = newId
      } else if (level === 3) {
        // Child of previous level-2 child - use letters again
        if (!lastLevel2Id) {
          new Notice('Error: Cannot create level 3 child without a level 2 parent')
          continue
        }
        ...
        new Notice(`Error: Invalid nesting level ${level}. Maximum depth is 3.`)
        continue
      }
      ...
      const prefix = this.plugin.settings.useZettelPrefix ? this.plugin.settings.zettelPrefix : ''
