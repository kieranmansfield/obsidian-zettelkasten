...
export class ZettelkastenView extends ItemView {
  ...
  private renderContent(container: HTMLElement): void {
    ...
    if (this.plugin.settings.enableProjects) {
      menuItems.push({
        ...
        icon: 'folder-kanban',
        folderName: this.plugin.settings.projectsLocation || 'Projects',
    ...
    if (this.plugin.settings.enableNoteSequence && this.plugin.settings.panelShowNoteSequence) {
  ...
  private createMenuItem(container: HTMLElement, item: MenuItem): void {
    ...
    const isDashboardActive = item.dashboardPath && activeFile?.path === item.dashboardPath
  ...
  private isDashboardFile(file: TFile | null): boolean {
    ...
    return dashboardPaths.some((dashboardPath) => {
      ...
      return dashboardFile instanceof TFile && dashboardFile.path === file.path
    })
  }
