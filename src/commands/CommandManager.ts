...
export class CommandManager {
	...
	/**
	 * Gets the box that a file belongs to based on folder or tag
	 */
	private getBoxForFile(file: TFile): Box {
		for (const box of this.plugin.settings.boxes) {
			if (this.fileMatchesBox(file, box)) {
				return box;
			}
		}
		// Default to first box if no match
		return this.plugin.settings.boxes[0];
	}

	/**
	 * Gets the default (first) box, or creates a virtual box from global settings if boxes are disabled
	 */
	private getDefaultBox(): Box {
		if (!this.plugin.settings.enableBoxes) {
			// Return a virtual box using global settings
			const s = this.plugin.settings;
			return {
				id: "global",
				name: "Global Settings",
				type: "folder",
				folderPath: s.zettelsLocation,
				useBoxPrefix: false,
				boxPrefix: "",
				zettelIdFormat: s.zettelIdFormat,
				useSeparatorFormat: s.useSeparatorFormat,
				zettelIdSeparator: s.zettelIdSeparator,
				zettelIdMatchingMode: s.zettelIdMatchingMode,
				noteTemplatePath: s.noteTemplatePath,
				zettelTag: s.zettelTag,
				enableSequenceReorder: s.enableSequenceReorder,
				useZettelPrefix: s.useZettelPrefix,
				zettelPrefix: s.zettelPrefix,
				enableFleetingNotes: s.enableFleetingNotes,
				fleetingNotesUseSeparateLocation: s.fleetingNotesUseSeparateLocation,
				fleetingNotesLocation: s.fleetingNotesLocation,
				fleetingNotesTemplatePath: s.fleetingNotesTemplatePath,
				fleetingNotesUseZettelId: s.fleetingNotesUseZettelId,
				fleetingNotesFilenameFormat: s.fleetingNotesFilenameFormat,
				fleetingNotesTag: s.fleetingNotesTag,
				useFleetingNotesPrefix: s.useFleetingNotesPrefix,
				fleetingNotesPrefix: s.fleetingNotesPrefix,
				enableMocs: s.enableMocs,
				mocsUseSeparateLocation: s.mocsUseSeparateLocation,
				mocsLocation: s.mocsLocation,
				mocsTemplatePath: s.mocsTemplatePath,
				mocsUseZettelId: s.mocsUseZettelId,
				mocsFilenameFormat: s.mocsFilenameFormat,
				mocsTag: s.mocsTag,
				useMocsPrefix: s.useMocsPrefix,
				mocsPrefix: s.mocsPrefix,
				enableIndexes: s.enableIndexes,
				indexesUseSeparateLocation: s.indexesUseSeparateLocation,
				indexesLocation: s.indexesLocation,
				indexesTemplatePath: s.indexesTemplatePath,
				indexesUseZettelId: s.indexesUseZettelId,
				indexesFilenameFormat: s.indexesFilenameFormat,
				indexesTag: s.indexesTag,
				useIndexesPrefix: s.useIndexesPrefix,
				indexesPrefix: s.indexesPrefix,
	...
	private registerGlobalCommands(): void {
		...
		if (this.getDefaultBox().enableSequenceReorder) {
	...
	private registerOpenFleetingCommand(): void {
		this.plugin.addCommand({
			...
			icon: "file-text",
			checkCallback: (checking: boolean) => {
				if (!this.getDefaultBox().enableFleetingNotes) {
					return false;
				}
				...
				this.getNoteTitlesByTag(
					this.getDefaultBox().fleetingNotesTag,
				).then((titles) => {
					new ZettelSuggester(
						this.plugin.app,
						titles,
	...
	private registerOpenMocCommand(): void {
		this.plugin.addCommand({
			...
			icon: "file-stack",
			checkCallback: (checking: boolean) => {
				if (!this.getDefaultBox().enableMocs) {
					return false;
				}
				...
				this.getNoteTitlesByTag(this.getDefaultBox().mocsTag).then(
					(titles) => {
						new ZettelSuggester(
							this.plugin.app,
							titles,
	...
	private registerOpenIndexCommand(): void {
		this.plugin.addCommand({
			...
			icon: "list",
			checkCallback: (checking: boolean) => {
				if (!this.getDefaultBox().enableIndexes) {
					return false;
				}
				...
				this.getNoteTitlesByTag(this.getDefaultBox().indexesTag).then(
					(titles) => {
						new ZettelSuggester(
							this.plugin.app,
							titles,
	...
	private registerAssignParentCommand(): void {
		this.plugin.addCommand({
			...
			checkCallback: (checking: boolean) => {
				...
				this.getNoteTitlesByTag(this.getDefaultBox().zettelTag).then(
					(titles) => {
			...
			checkCallback: (checking: boolean) => {
				...
				(async () => {
					try {
						...
						// Get the folder where zettels are stored
						const folder = this.getNoteTypeFolder("zettel", this.getDefaultBox());
	...
	private isZettelByTag(file: TFile, box?: Box): boolean {
		...
		const zettelTag = box?.zettelTag ?? this.getDefaultBox().zettelTag;
				...
				new CreateNoteWithSuggestModal(
					...
					async (title: string) => {
						...
						const creator = new FileCreator(
							...
							this.getDefaultBox().noteTemplatePath,
						);
						try {
	...
	private registerCreateFleetingNoteCommand(): void {
		this.plugin.addCommand({
			...
			callback: async () => {
				...
				const folder = this.getNoteTypeFolder("fleeting", this.getDefaultBox());
				...
				const creator = new FileCreator(
					...
					this.getDefaultBox().fleetingNotesTemplatePath,
				);
				try {
					await creator.create();
				} catch (error) {
						...
						new CreateNoteWithSuggestModal(
							...
							const folder = this.getNoteTypeFolder(
								...
								const creator = new FileCreator(
									...
									this.getDefaultBox().mocsTemplatePath,
								);
								try {
									await creator.create();
								} catch (error) {
						...
						new CreateNoteWithSuggestModal(
							...
							const folder = this.getNoteTypeFolder(
								...
								const creator = new FileCreator(
									...
									this.getDefaultBox().indexesTemplatePath,
								);
								try {
									await creator.create();
								} catch (error) {
		...
		this.plugin.addCommand({
			...
			checkCallback: (checking: boolean) => {
				...
				const hasSeparateLocations =
					this.getDefaultBox().folderPath !== "" ||
					this.getDefaultBox().fleetingNotesUseSeparateLocation ||
					this.getDefaultBox().mocsUseSeparateLocation ||
					this.getDefaultBox().indexesUseSeparateLocation;
	...
	private async moveFileToCorrectLocation(file: TFile): Promise<boolean> {
		...
		switch (noteType) {
			case "fleeting":
				targetFolder = this.getNoteTypeFolder("fleeting", this.getDefaultBox());
				break;
			case "moc":
				targetFolder = this.getNoteTypeFolder("moc", this.getDefaultBox());
				break;
			case "index":
				targetFolder = this.getNoteTypeFolder("index", this.getDefaultBox());
				break;
			default: // zettel
				targetFolder = this.getNoteTypeFolder("zettel", this.getDefaultBox());
		}
	...
	private async tagFileAsCorrectType(file: TFile): Promise<boolean> {
		...
		switch (noteType) {
			case "fleeting":
				targetTag = this.getDefaultBox().fleetingNotesTag;
				break;
			case "moc":
				targetTag = this.getDefaultBox().mocsTag;
				break;
			case "index":
				targetTag = this.getDefaultBox().indexesTag;
				break;
			default: // zettel
				targetTag = this.getDefaultBox().zettelTag;
		}
	...
	private extractZettelId(filename: string): string | null {
		...
		const separator = this.getDefaultBox().zettelIdSeparator;
	...
	private getNoteTypeFolder(
		...
		switch (noteType) {
			case "fleeting":
				targetLocation = box.fleetingNotesUseSeparateLocation
					? box.fleetingNotesLocation
					: box.folderPath || "";
				break;
			case "moc":
				targetLocation = box.mocsUseSeparateLocation
					? box.mocsLocation
					: box.folderPath || "";
				break;
			case "index":
				targetLocation = box.indexesUseSeparateLocation
					? box.indexesLocation
					: box.folderPath || "";
				break;
			default: // zettel
				targetLocation = box.folderPath || "";
				break;
		}
	...
	private generateZettelId(
		...
		const format =
			customFormat || this.getDefaultBox().zettelIdFormat || "YYYYMMDDHHmmss";
		const now = new Date();
	...
	private getPrefixForNoteType(
		...
		if (box) {
			...
			switch (noteType) {
				case "zettel":
					prefix += this.getDefaultBox().zettelPrefix || "";
					break;
				case "fleeting":
					prefix += this.getDefaultBox().fleetingNotesPrefix || "";
					break;
				case "moc":
					prefix += this.getDefaultBox().mocsPrefix || "";
					break;
				case "index":
					prefix += this.getDefaultBox().indexesPrefix || "";
					break;
			}
		}
	...
	private shouldUsePrefixForNoteType(
		...
		if (box) {
			...
			switch (noteType) {
				case "zettel":
					hasNoteTypePrefix = this.getDefaultBox().useZettelPrefix;
					break;
				case "fleeting":
					hasNoteTypePrefix = this.getDefaultBox().useFleetingNotesPrefix;
					break;
				case "moc":
					hasNoteTypePrefix = this.getDefaultBox().useMocsPrefix;
					break;
				case "index":
					hasNoteTypePrefix = this.getDefaultBox().useIndexesPrefix;
					break;
			}
		}
	...
	private getNoteTypeFromFile(
		...
		const fleetingTag = box?.fleetingNotesTag ?? this.getDefaultBox().fleetingNotesTag;
		const mocTag = box?.mocsTag ?? this.getDefaultBox().mocsTag;
		const indexTag = box?.indexesTag ?? this.getDefaultBox().indexesTag;
	...
	private buildZettelFilename(zettelId: string, title: string, box?: Box): string {
		const useSeparatorFormat = box?.useSeparatorFormat ?? this.getDefaultBox().useSeparatorFormat;
		const zettelIdSeparator = box?.zettelIdSeparator ?? this.getDefaultBox().zettelIdSeparator;
	...
	private buildFleetingFilename(title: string, box?: Box): string {
		const fleetingNotesUseZettelId = box?.fleetingNotesUseZettelId ?? this.getDefaultBox().fleetingNotesUseZettelId;
		const fleetingNotesFilenameFormat = box?.fleetingNotesFilenameFormat ?? this.getDefaultBox().fleetingNotesFilenameFormat;
	...
	private buildMocFilename(title: string, box?: Box): string {
		const mocsUseZettelId = box?.mocsUseZettelId ?? this.getDefaultBox().mocsUseZettelId;
		const mocsFilenameFormat = box?.mocsFilenameFormat ?? this.getDefaultBox().mocsFilenameFormat;
	...
	private buildIndexFilename(title: string, box?: Box): string {
		const indexesUseZettelId = box?.indexesUseZettelId ?? this.getDefaultBox().indexesUseZettelId;
		const indexesFilenameFormat = box?.indexesFilenameFormat ?? this.getDefaultBox().indexesFilenameFormat;
