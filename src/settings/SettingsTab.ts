...
export class ZettelkastenSettingTab extends PluginSettingTab {
  ...
  private createCollapsibleSection(
    ...
    toggleConfig?: { getValue: () => boolean; onChange: (value: boolean) => Promise<void> }
  ): HTMLElement {
    const details = containerEl.createEl('details', { cls: 'zettelkasten-collapsible-section' })
    ...
    if (toggleConfig) {
      ...
      const setting = new Setting(toggleDiv).addToggle((toggle) =>
        toggle.setValue(toggleConfig.getValue()).onChange(toggleConfig.onChange)
      )
    ...
    contentDiv.createEl('p', { text: description, cls: 'setting-item-description' })
    return contentDiv
  }
  ...
  private displayGeneralSettings(containerEl: HTMLElement): void {
    ...
    new Setting(sectionDiv)
      ...
      .setDesc('Default folder path where new notes will be created (leave empty for vault root)')
      .addText((text) => {
    ...
    const ignoredFoldersSetting = new Setting(sectionDiv)
      ...
      .setDesc('Folder paths to exclude from commands. Click + to add more folders.')
    ...
    this.plugin.settings.ignoredFolders.forEach((folder, index) => {
      new Setting(sectionDiv)
        ...
        .addText((text) => {
          ...
          new FolderSuggest(this.app, text.inputEl, onSelect)
          text.setPlaceholder('folder/path').setValue(folder).onChange(onSelect)
        })
        .addButton((button) => {
          button
            .setIcon('trash')
  ...
  private displayZettelSettings(containerEl: HTMLElement): void {
    ...
    new Setting(sectionDiv)
      ...
      .setDesc('How to identify zettel notes: by folder location or by tag')
    ...
    new Setting(sectionDiv)
      ...
      .setDesc('Folder path for zettel notes (leave empty for vault root)')
      .addText((text) => {
        const onSelect = async (value: string) => {
          text.setValue(value)
        ...
        new FolderSuggest(this.app, text.inputEl, onSelect)
        text
          .setPlaceholder('path/to/zettels')
          .setValue(this.plugin.settings.zettelsLocation)
          .onChange(onSelect)
      })
    new Setting(sectionDiv)
      .setName('Zettel ID format')
      .setDesc('Date format for generating zettel IDs (e.g., YYYYMMDDHHmm, YYYYMMDD)')
      .addText((text) =>
        text
    ...
    new Setting(sectionDiv)
      ...
      .setDesc('Path to the template file for new notes (leave empty to use default)')
      .addText((text) => {
        const onSelect = async (value: string) => {
          text.setValue(value)
    ...
    new Setting(sectionDiv)
      ...
      .addText((text) => {
        ...
        text.setPlaceholder('zettel').setValue(this.plugin.settings.zettelTag).onChange(onSelect)
      })
  }

  // Modular methods for other settings sections
  private displayNoteSequenceSettings(containerEl: HTMLElement): void {
    // Implement modular Note Sequence settings using createCollapsibleSection
    // ...
  }
  private displayInboxSettings(containerEl: HTMLElement): void {
    // Implement modular Inbox settings using createCollapsibleSection
    // ...
  }
  private displayStructureNoteSettings(containerEl: HTMLElement): void {
    // Implement modular Structure Note settings using createCollapsibleSection
    // ...
  }
  private displayReferenceSettings(containerEl: HTMLElement): void {
    // Implement modular Reference settings using createCollapsibleSection
    // ...
  }
  private displayProjectsSettings(containerEl: HTMLElement): void {
    // Implement modular Projects settings using createCollapsibleSection
    // ...
  }
  private displayPanelSettings(containerEl: HTMLElement): void {
    // Implement modular Panel settings using createCollapsibleSection
    // ...
  }
}
...
//         .setDesc('Show note sequences view with cards for all parent notes and their children.')
//         .addToggle((toggle) =>
//           toggle.setValue(this.plugin.settings.enableNoteSequencesView).onChange(async (value) => {
...
//         .setDesc('Show the current note sequence navigator in the right sidebar.')
//         .addToggle((toggle) =>
//           toggle.setValue(this.plugin.settings.enableSequenceNavigator).onChange(async (value) => {
...
//               const leaves = workspace.getLeavesOfType('sequence-navigator-view')
...
//         .setDesc('Folder path for inbox notes (leave empty for vault root)')
//         .addText((text) => {
//           const onSelect = async (value: string) => {
//             text.setValue(value)
...
//             const onSelect = async (value: string) => {
//               text.setValue(value)
//               this.plugin.settings.defaultInboxTemplatePath = value
//               await this.plugin.saveSettings()
//             }
//             new FileSuggest(this.app, text.inputEl, onSelect)
//             text
//               .setPlaceholder('templates/inbox.md')
//               .setValue(this.plugin.settings.defaultInboxTemplatePath)
//               .onChange(async (value) => {
//                 this.plugin.settings.defaultInboxTemplatePath = value
//                 await this.plugin.saveSettings()
//               })
...
//           .addToggle((toggle) =>
//             toggle
//               .setValue(this.plugin.settings.fleetingNotesUseSeparateLocation)
//               .onChange(async (value) => {
//                 this.plugin.settings.fleetingNotesUseSeparateLocation = value
//                 await this.plugin.saveSettings()
...
//             .setDesc('Folder path for fleeting notes (leave empty for vault root)')
//             .addText((text) => {
//               const onSelect = async (value: string) => {
//                 text.setValue(value)
...
//               new FolderSuggest(this.app, text.inputEl, onSelect)
//               text
//                 .setPlaceholder('path/to/fleeting')
//                 .setValue(this.plugin.settings.fleetingNotesLocation)
//                 .onChange(async (value) => {
//                   this.plugin.settings.fleetingNotesLocation = value
//                   await this.plugin.saveSettings()
//                 })
...
//             new FileSuggest(this.app, text.inputEl, onSelect)
//             text
//               .setPlaceholder('templates/fleeting-note.md')
//               .setValue(this.plugin.settings.fleetingNotesTemplatePath)
//               .onChange(async (value) => {
//                 this.plugin.settings.fleetingNotesTemplatePath = value
//                 await this.plugin.saveSettings()
//               })
...
//               .onChange(async (value) => {
//                 this.plugin.settings.fleetingNotesUseZettelId = value
//                 await this.plugin.saveSettings()
...
//             .setDesc('Custom filename format (use {{title}} as placeholder)')
//             .addText((text) =>
//               text
//                 .setPlaceholder('{{title}}')
//                 .setValue(this.plugin.settings.fleetingNotesFilenameFormat)
//                 .onChange(async (value) => {
//                   this.plugin.settings.fleetingNotesFilenameFormat = value
//                   await this.plugin.saveSettings()
//                 })
//             )
//         }
...
//         .setDesc('Choose between Map of Content (MOC) or Zettelkasten Index style')
//         .addDropdown((dropdown) =>
//           dropdown
...
//         .addToggle((toggle) =>
//           toggle
//             .setValue(this.plugin.settings.structureNotesUseSeparateLocation)
//             .onChange(async (value) => {
//               this.plugin.settings.structureNotesUseSeparateLocation = value
//               await this.plugin.saveSettings()
//               this.display() // Refresh to show/hide location field
//             })
//         )
...
//           .setDesc('Folder path for structure notes (leave empty for vault root)')
//           .addText((text) => {
//             const onSelect = async (value: string) => {
//               text.setValue(value)
...
//             new FolderSuggest(this.app, text.inputEl, onSelect)
//             text
//               .setPlaceholder('path/to/structure-notes')
//               .setValue(this.plugin.settings.structureNotesLocation)
//               .onChange(async (value) => {
//                 this.plugin.settings.structureNotesLocation = value
//                 await this.plugin.saveSettings()
//               })
...
//           .setDesc('Path to template file for Zettelkasten Index notes')
//           .addText((text) => {
//             const onSelect = async (value: string) => {
//               text.setValue(value)
...
//             new FileSuggest(this.app, text.inputEl, onSelect)
//             text
//               .setPlaceholder('templates/zk-index.md')
...
//       new Setting(contentDiv)
//         .setName('Use zettel ID format')
//         .setDesc('Use the zettel ID format for structure note filenames')
//         .addToggle((toggle) =>
//           toggle
//             .setValue(this.plugin.settings.structureNotesUseZettelId)
//             .onChange(async (value) => {
//               this.plugin.settings.structureNotesUseZettelId = value
//               await this.plugin.saveSettings()
//               this.display() // Refresh to show/hide format field
//             })
//         )
...
//               .setValue(this.plugin.settings.structureNotesFilenameFormat)
//               .onChange(async (value) => {
//                 this.plugin.settings.structureNotesFilenameFormat = value
//                 await this.plugin.saveSettings()
//               })
//           )
//       }
...
//         .setDesc('Folder path for reference materials (leave empty for vault root)')
//         .addText((text) => {
//           const onSelect = async (value: string) => {
//             text.setValue(value)
...
//           new FolderSuggest(this.app, text.inputEl, onSelect)
//           text
//             .setPlaceholder('path/to/reference')
...
//         .setDesc('Folder path for projects (leave empty for vault root)')
//         .addText((text) => {
//           const onSelect = async (value: string) => {
//             text.setValue(value)
...
//           new FolderSuggest(this.app, text.inputEl, onSelect)
//           text
//             .setPlaceholder('path/to/projects')
...
//           )
//           .addToggle((toggle) =>
//             toggle.setValue(this.plugin.settings.panelShowNoteSequence).onChange(async (value) => {
...
//         )
//         .addToggle((toggle) =>
//           toggle.setValue(this.plugin.settings.panelShowFileLists).onChange(async (value) => {
...
//         .addToggle((toggle) =>
//           toggle.setValue(this.plugin.settings.panelShowFileIcons).onChange(async (value) => {
...
//               .onChange(async (value) => {
//                 this.plugin.settings.panelProjectsName = value || 'Projects'
//                 await this.plugin.saveSettings()
//               })
//           )
//       }
...
//         .setDesc('When multiple tags are specified, match files with ANY tag or ALL tags')
//         .addDropdown((dropdown) =>
//           dropdown
...
//         .setDesc('Path to the file that opens when clicking the Inbox heading')
//         .addText((text) => {
//           const onSelect = async (value: string) => {
//             text.setValue(value)
...
//               .onClick(async () => {
//                 this.plugin.settings.panelInboxFilterTags.splice(index, 1)
//                 await this.plugin.saveSettings()
//                 this.display()
//               })
//           })
...
//         .setDesc('Path to the file that opens when clicking the Zettels heading')
//         .addText((text) => {
//           const onSelect = async (value: string) => {
//             text.setValue(value)
...
//               .setTooltip('Remove tag')
//               .onClick(async () => {
//                 this.plugin.settings.panelZettelsFilterTags.splice(index, 1)
//                 await this.plugin.saveSettings()
//                 this.display()
//               })
//           })
...
//         .setDesc('Path to the file that opens when clicking the References heading')
//         .addText((text) => {
//           const onSelect = async (value: string) => {
//             text.setValue(value)
...
//           new FileSuggest(this.app, text.inputEl, onSelect)
//           text
//             .setPlaceholder('References/References.md')
...
//               .setTooltip('Remove tag')
//               .onClick(async () => {
//                 this.plugin.settings.panelReferencesFilterTags.splice(index, 1)
//                 await this.plugin.saveSettings()
//                 this.display()
//               })
//           })
...
//           .setDesc('Path to the file that opens when clicking the Projects heading')
//           .addText((text) => {
//             const onSelect = async (value: string) => {
//               text.setValue(value)
...
//             new FileSuggest(this.app, text.inputEl, onSelect)
//             text
//               .setPlaceholder('Projects/Projects.md')
...
//               const onSelect = async (value: string) => {
//                 text.setValue(value)
//                 this.plugin.settings.panelProjectsFilterTags[index] = value
//                 await this.plugin.saveSettings()
//               }
//               new TagSuggest(this.app, text.inputEl, onSelect)
//               text
//                 .setPlaceholder('#project')
//                 .setValue(this.plugin.settings.panelProjectsFilterTags[index])
//                 .onChange(async (value) => {
//                   this.plugin.settings.panelProjectsFilterTags[index] = value
//                   await this.plugin.saveSettings()
//                 })
...
//                 .setTooltip('Remove tag')
//                 .onClick(async () => {
//                   this.plugin.settings.panelProjectsFilterTags.splice(index, 1)
//                   await this.plugin.saveSettings()
//                   this.display()
//                 })
//             })
...
//         .setName('Bookmarks')
//         .setDesc('Manage your bookmarked files. Click + to add a bookmark.')
...
//           .onClick(async () => {
//             const modal = new BookmarkModal(this.app, this.plugin, async (bookmark) => {
...
//         const bookmarkSetting = new Setting(contentDiv).setClass('zettelkasten-ignored-folder-item')
